...
if __name__ == '__main__':
	...
	checkpoint_list = glob.glob("../../transfer/weights-enh-e4/vgg_rjig33(1.000000)/trainer_*.pth")
	checkpoint_list.sort()
	checkpoint_list = checkpoint_list[20:]

	for checkpoint in checkpoint_list:
		print("Current:", checkpoint)
		net_param = torch.load(checkpoint)

		pre_processor = EnhanceNet(2).cuda()
		pre_processor.eval()
		...
		pre_processor.load_state_dict(new_net_param)
		...
		try:
			...
			for i_round in tqdm(range(total_round)):
				i_batch = 0
				...
				for data, target, video_subpath in eval_iter:
					...
					# vutils.save_image(NormBack(data.reshape(data.shape[0], data.shape[1], -1, data.shape[4])), 'before.png', normalize=False)
					data = pre_processor(data.float().cuda())
					# vutils.save_image(NormBack(data.reshape(data.shape[0], data.shape[1], -1, data.shape[4])), 'after.png', normalize=False)
