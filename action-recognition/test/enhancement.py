...
class depthwise_separable_conv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, 
        ...
        self.depth_wise = nn.Conv2d(in_channels, in_channels, kernel_size, stride,
...
class CurvePredictor(nn.Module):
    ...
    def __init__(self, P=256, C=3, grad_degree=2, number_f=32):
        super(CurvePredictor, self).__init__()
        self.relu = nn.ReLU(inplace=True)
        
        self.e_conv1 = depthwise_separable_conv(C,number_f,3,1,1,bias=True) 
        self.e_conv2 = depthwise_separable_conv(number_f,number_f,3,1,1,bias=True) 
        self.e_conv3 = depthwise_separable_conv(number_f,number_f,3,1,1,bias=True) 
        self.e_conv4 = depthwise_separable_conv(number_f,number_f,3,1,1,bias=True) 
        self.e_conv5 = depthwise_separable_conv(number_f,number_f,3,1,1,bias=True)
        
        self.d_conv1 = depthwise_separable_conv(number_f,number_f*2,3,2,1,bias=True)
        self.d_conv2 = depthwise_separable_conv(number_f*2,number_f*4,3,2,1,bias=True)
        self.d_linear = nn.Linear(number_f*4, (P-grad_degree)*C, bias=True)

    def forward(self, x, return_curve=False):
        x1 = self.relu(self.e_conv1(x))
        x2 = self.relu(self.e_conv2(x1))
        x3 = self.relu(self.e_conv3(x2))
        x4 = self.relu(self.e_conv4(x2+x3))
        x5 = self.relu(self.e_conv5(x1+x4))
        ...
        return self.relu(self.d_linear(x7))
