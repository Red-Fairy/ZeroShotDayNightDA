...
parser.add_argument('--task-name', type=str, default='action-modify',
...
def set_logger(debug_mode=False):
	...
	logging.basicConfig(level=logging.DEBUG if debug_mode else log.info,
...
if __name__ == '__main__':
	...
	log_dir = os.path.join(args.model_dir, f'test-load{args.load_epoch}-rounds{args.test_rounds}')
	...
	set_logger(debug_mode=args.debug_mode)
	log.info("Start evaluation with args:\n" +
	...
	if args.AdaBN:
		for module in net.net.modules():
			if 'BatchNorm3d' in module.__class__.__name__:
	...
	for i_round in range(total_round):
		i_batch = 0
		log.info("round #{}/{}".format(i_round, total_round))
		for data, target, video_subpath in eval_iter:
			...
			# log.info("output is {}, target is {}".format(output, target))
			for i_item in range(0, output.shape[0]):
				...
				pred = output_i.argmax(dim=1, keepdim=False)
				# if pred == target_i:
				# 	print(video_subpath_i, "correct")
				# 	right_dict[video_subpath_i] = [pred.item(), output_i[0,pred].item()]
				# else:
				# 	print(video_subpath_i, "wrong")
				# 	wrong_dict[video_subpath_i] = [pred.item(), output_i[0,pred].item()]
			...
			if (i_batch % 100) == 0:
				...
				log.info("{:.1f}%, {:.1f} \t| Batch [0,{}]    \tAvg: {} = {:.5f}, {} = {:.5f}, {} = {:.5f}".format(
	...
	# pickle.dump(right_dict, open("result_lists/ours_right_list.pkl", "wb"))
	# pickle.dump(wrong_dict, open("result_lists/ours_wrong_list.pkl", "wb"))
	# finished
	...
	log.info("Total time cost: {:.1f} sec".format(sum_batch_elapse))
	log.info("Speed: {:.4f} samples/sec".format(
			args.batch_size * sum_batch_inst / sum_batch_elapse ))
	log.info("Accuracy:")
	log.info(json.dumps(metrics.get_name_value(), indent=4, sort_keys=True))
